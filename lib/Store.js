...
Store.prototype.read = function (model, options) {
    ...
    if(options.filter) {
        _.each(options.filter, function (val, key) {
            var obj = {};
            obj[key]=val;
    ...
    if(options.clauses) {
        _.each(options.clauses, function (clause) {
    ...
    if(options.rawFields) {
        _.each(options.rawFields, function (field) {
            fields.push(field);
        });
    }
...
Store.prototype.update = function (model, options) {
    ...
    var query = util.format('UPDATE %s SET doc = $1 WHERE id = $2', model.table);
        values = []
        object = model.toJSON()
    ...
    pg.connect(this.connection, function (err, client) {
        if(err) return options.error(err);
        client.query(query, values, function (err, info) {
            if(err) return options.error(err);
            options.success(model.toJSON());
        });
    });
...
Store.prototype.delete = function (model, options) {
    var query = util.format('DELETE FROM %s WHERE id = $1', model.table);
        values = [model.id]
        object = model.toJSON()
